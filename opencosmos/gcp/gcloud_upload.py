        ...
        if os.path.exists(self.abs_filepath):
            ...
            try:
                ...
                if self.filename not in self.bucket_files:
                    ...
                    # Uploading file
                    sp.check_call(
                        f"gsutil cp -r {self.abs_filepath} gs://{self.bucket_path}",
                        shell=True,
                        stdout=sp.PIPE,
                    )
                    loading.stop()
...
def main(project_id, local_filepath, bucket_path):
    ...
    gcloud = Google_Cloud_Storage(
        log=logging, project_id=project_id, local_filepath=local_filepath, bucket_path=bucket_path
    )
    gcloud.upload()
